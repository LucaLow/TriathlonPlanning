...
app.post("/login", (req, res) => {
  ...
  const query = mysql.format("SELECT Password FROM Users WHERE UserName = ?;", [
    username,
  ]);
  connection.query(query, function (err, result, fields) {
    if (result.length === 0) {
      return res
        .status(401)
        .json({ message: "Authentication failed, login failure" });
    }
...
app.post("/GetEvent", verifyToken, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error getting list", error: error });
  }
});
...
app.post("/CreateEvent", verifyToken, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error creating Event", error: error });
  }
});
...
app.post("/RemoveEvent", verifyToken, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error removing event", error: error });
  }
});
...
function checkIfUserExists(username) {
  return new Promise((resolve, reject) => {
    const query = mysql.format("SELECT * FROM Users WHERE UserName = ?;", [
      username,
    ]);
    connection.query(query, function (err, result, fields) {
      if (err) reject(err);
...
function RemoveList(id) {
  return new Promise((resolve, reject) => {
    removeEventQuery = mysql.format("DELETE FROM `Event` WHERE EventID = ?;", [
      id,
    ]);
...
function CreateList(name, description, id) {
  return new Promise((resolve, reject) => {
    let query = mysql.format(
      "INSERT INTO `Shopping List` (`Name`, `Description`, `Owner_ID`) VALUES (?, ?, ?);",
      [name, description, id]
    );
    connection.query(query, function (err, result, fields) {
      if (err) reject(err);
...
function GetListData(UserID) {
  return new Promise((resolve, reject) => {
    let query = mysql.format("SELECT * FROM `Event` WHERE `UserID` = ?", [
      UserID,
    ]);
...
function getIdFromUsername(username) {
  return new Promise((resolve, reject) => {
    let query = mysql.format("SELECT ID FROM Users WHERE UserName = ?;", [
      username,
    ]);
    connection.query(query, function (err, result, fields) {
      if (err) reject(err);

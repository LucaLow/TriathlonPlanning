...
function CalendarView() {
  ...
  function removeEvent(sellectedActivity) {
    ...
    setTrainingData(
      trainingData.filter((activity) => activity !== sellectedActivity)
    );
  }
  ...
  const cellRender = (current, info) => {
    ...
    const activities = trainingData.filter(
      (activity) => dayjs(activity.Date).format("YYYY-MM-DD") === formattedDate
    );
    return (
      <ul
        className="events"
      ...
      <div className="CardHolder">
        {Array.isArray(trainingData) &&
          trainingData
            .filter(
              (activity) =>
                dayjs(activity.Date).format("YYYY-MM-DD") === sellectedDate
            )
            .map((activity, index) => (
              <Card
